function.Regression_Analysis <- function(Predefined_lists, rv){
  # Lists
...
'
  ...
  regression_reference <- list(
    ...
    plots_list = list(list("correlation_plot", "Bland_Altman_plot", "actual_vs_predicted_plot" ,"residuals_vs_fitted", "normality_residuals", "standardised_residuals", "leverage", "composite_plot"),list("correlation_plot", "actual_vs_predicted_plot", "ROC_plot", "leverage", "composite_plot"),list("correlation_plot", "actual_vs_predicted_plot", "composite_plot"),list("correlation_plot", "actual_vs_predicted_plot", "composite_plot"),list("correlation_plot", "Bland_Altman_plot", "residuals_vs_fitted", "composite_plot"),list("correlation_plot", "actual_vs_predicted_plot", "ROC_plot", "residuals_vs_fitted", "composite_plot"))
  )
  regression_text_1 <- paste0(
    ...
    ifelse(length(mandatory_predictors) == 0, 1, paste0("`", mandatory_predictors, "`", collapse = ' + ')),
  ...
  if (TRUE %in% (str_detect(regression_results_1[[1]][1],"Error"))) {
    results <- data.frame(
      ...
      `Reason` = "The analysis did not run correctly even with mandatory variables. Please try with fewer mandatory variables.",
      check.names = FALSE
    )
    ...
    if (is.null(regression_results_1$convergence)) {
      ...
      convergence <- (regression_results_1$convergence == 0)
    }
    if (convergence == FALSE){
      ...
      if (length(optional_predictors) == 0) {
        ...
        regression_results_2 <- suppressMessages(suppressWarnings(try(eval(parse(text = paste0(
          "stats::step(regression_results_1, scope = list(upper = ~ ",
          paste0("`", c(mandatory_predictors, optional_predictors), "`", collapse = operator), ", ",
          "lower = ~ ", ifelse(length(mandatory_predictors) == 0, 1, paste0("`", mandatory_predictors, "`", collapse = ' + ')),
          "), trace = FALSE)"
        ))), silent = TRUE)))
      ...
      regression_coefficients <- data.frame(summary(regression_results_2)$coefficients, check.rows = FALSE, check.names = FALSE)
      ...
      if (regression_type == "Linear regression") {
        ...
        regression_fit_statistics <- cbind.data.frame(
          `Residual Standard Error` = summary(regression_results_2)$sigma,
          `Degrees of freedom (Residual Standard Error)` = summary(regression_results_2)$df[2],
          `Adjusted R-square` = summary(regression_results_2)$adj.r.squared,
          `AIC` = AIC(regression_results_2),
          `BIC` = BIC(regression_results_2),
          `F statistic` = summary(regression_results_2)$fstatistic[1],
          `Degrees of freedom (F statistic) ` = paste0(summary(regression_results_2)$fstatistic[2], "; ", summary(regression_results_2)$fstatistic[3]),
          `P value` = pf(summary(regression_results_2)$fstatistic[1], summary(regression_results_2)$fstatistic[2], summary(regression_results_2)$fstatistic[3], lower.tail = FALSE, log.p = FALSE),
        ...
        regression_coefficients <- t(summary(regression_results_2)$coefficients)
        # Standard errors
        regression_se <- t(summary(regression_results_2)$standard.errors)
        # Odds ratios
        ...
        predicted_values <- data.frame(
          `Actual values` = data[,1],
          `Predicted values` = predict(regression_results_2, newdata = data, type = "class"),
          check.names = FALSE
        )
        actual_vs_predicted <- data.frame(unclass(table(predicted_values)), check.names = FALSE)
        actual_vs_predicted <- cbind.data.frame(
        ...
        if (str_detect(model_comparison[[1]][1], "Error")) {
          ...
          if (str_detect(model_comparison[[1]][1], "Error")) {
            regression_fit_statistics <- cbind.data.frame(
              ...
              `Log rank test P value` = summary(regression_results_2)$sctest[3],
              `Wald test P value` = summary(regression_results_2)$waldtest[3]
            )
          } else {
            regression_fit_statistics <- cbind.data.frame(
              `AIC` = AIC(regression_results_2),
      ...
      if ((regression_type == "Logistic regression") | (regression_type == "Multinomial logistic regression") | (regression_type == "Ordinal logistic regression") | (regression_type == "Cox regression")) {
      ...
      plot_title_combined <- ggdraw() + draw_label(paste0(rv$entry[[1]], '(', regression_type, ')'), color="darkgreen", size=14, fontface="bold", hjust = 0.5) + theme(plot.margin = margin(0, 0, 0, 7))
      ...
      if (regression_type == "Linear regression") {
        ...
        normality_residuals <- ggplot(data=regression_results_2, aes(sample = rstandard(regression_results_2))) + stat_qq() +  stat_qq_line(linetype=2, color="grey")+ ggtitle("Q-Q plot") + theme(plot.title = element_text(hjust=0.5)) + xlab("Theoretical quantiles")+ ylab("Sample quantiles")
        standardised_residuals <- ggplot(data=regression_results_2, aes(x= fitted(regression_results_2), y=sqrt(abs(rstandard(regression_results_2))))) + geom_point(na.rm=TRUE) + stat_smooth(method="loess", na.rm = TRUE) + xlab("Fitted Values") + ylab(expression(sqrt("Standardised residuals"))) + ggtitle("Scale-Location") + theme(plot.title = element_text(hjust=0.5))
      ...
      plot_title <- paste0(rv$entry[[1]], '_', regression_type)
